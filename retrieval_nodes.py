...
def node_rerank(state: AegisState) -> dict:
    ...
    try:
        ...
        reranker = CrossEncoderReranker(model=model, top_n=FINAL_K)
        ranked   = reranker.compress_documents(lc_docs, query)
        scores   = [float(d.metadata.get("relevance_score", 0.0)) for d in ranked]
        ce_error = None
    except Exception as exc:
...
_GEN_SYSTEM = (
    ...
    "Be precise, cite section names when available, and never hallucinate.\n"
...
def node_generate(state: AegisState) -> dict:
    ...
    return {
        "final_answer": answer,
        "messages": extra_messages + [sys_msg, user_msg, ai_msg, log],
    }
