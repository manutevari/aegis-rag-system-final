...
class AegisState(BaseModel):
    ...
    # ── Hallucination / fallback tracking ────────────────────────────────
    hallucination_risk: bool = Field(
        default=False,
        description="True if post-generation hallucination check detected foreign tokens.",
    )
    fallback_reason: str = Field(
        default="",
        description="Non-empty if a fallback was triggered; contains FallbackReason value.",
    )
    precision_at_k: dict = Field(
        default_factory=dict,
        description="Precision@1/3/5 computed from returned sources.",
    )
    retrieval_accuracy: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="Fraction of top-K sources containing at least one query token.",
    )
